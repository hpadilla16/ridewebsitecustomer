...
export default function ConfirmationPage() {
  ...
  const bookingType = confirmation?.bookingType === 'CAR_SHARING' ? t('common.carSharing') : t('common.rent');
  ...
  const paymentCard = buildActionCard(paymentAction, t('confirmation.payNow'), t('confirmation.payNowBody'), 'payment');
  const customerCard = buildActionCard(customerInfoAction, t('confirmation.completeInfo'), t('confirmation.completeInfoBody'), 'customer');
  const signatureCard = buildActionCard(signatureAction, t('confirmation.openSignature'), t('confirmation.openSignatureBody'), 'signature');
  ...
  const primaryActionBody = primaryAction?.variant === 'payment'
    ? (dueNow > 0 ? t('confirmation.continuePaymentAmount', { amount: fmtMoney(dueNow) }) : t('confirmation.continuePayment'))
    : primaryAction?.variant === 'customer'
      ? t('confirmation.finishCustomerInfo')
      : primaryAction?.variant === 'signature'
        ? t('confirmation.moveToSignature')
        : t('confirmation.continueFlow');

  if (!mounted) {
    return (
      <main style={{ minHeight: '60vh', display: 'grid', placeItems: 'center', padding: 40 }}>
        <div style={{ color: '#6b7a9a', fontWeight: 600 }}>{t('confirmation.loading')}</div>
      </main>
    );
  }
  ...
  if (!confirmation) {
    return (
      <main style={{ maxWidth: 560, margin: '0 auto', padding: '60px 20px', textAlign: 'center', display: 'grid', gap: 20 }}>
        ...
        <h1 style={{ fontSize: '1.6rem', fontWeight: 900, color: '#1e2847' }}>{t('confirmation.notFound')}</h1>
        <p style={{ color: '#6b7a9a', lineHeight: 1.7 }}>
          {t('confirmation.notFoundBody')}
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href={withSiteBase(basePath, '/car-sharing')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 48, padding: '0 24px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#7c3aed,#6e49ff 55%,#0fb0d8)', color: '#fff', fontWeight: 800, textDecoration: 'none', boxShadow: '0 10px 24px rgba(110,73,255,.28)' }}>{t('confirmation.browseMoreCars')}</Link>
          <Link href={withSiteBase(basePath, '/rent')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 48, padding: '0 20px', borderRadius: 14, border: '1.5px solid rgba(110,73,255,.18)', background: 'rgba(255,255,255,.9)', color: '#4a38be', fontWeight: 800, textDecoration: 'none' }}>{t('confirmation.rentalFleet')}</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(28px,5vw,52px) clamp(16px,4vw,24px) 60px', display: 'grid', gap: 24 }}>
      ...
      <div style={{ textAlign: 'center', display: 'grid', gap: 16, padding: '40px 24px', borderRadius: 24, background: 'linear-gradient(145deg, #f5f0ff, #faf7ff 60%, #f0fbf8)', border: '1px solid rgba(135,82,254,.1)' }}>
        ...
        <div>
          <h1 style={{ fontSize: 'clamp(1.6rem,4vw,2.2rem)', fontWeight: 900, letterSpacing: '-.02em', color: '#1a1230', margin: '0 0 10px' }}>{t('confirmation.allSet')}</h1>
          <p style={{ color: '#5f567e', lineHeight: 1.7, margin: 0 }}>{guestName !== '-' ? t('confirmation.reservationConfirmedName', { name: guestName, type: bookingType.toLowerCase() }) : t('confirmation.reservationConfirmed', { type: bookingType.toLowerCase() })}</p>
        </div>
        {/* Reference badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '12px 22px', borderRadius: 999, background: 'rgba(255,255,255,.9)', border: '1px solid rgba(135,82,254,.18)', margin: '0 auto', boxShadow: '0 4px 14px rgba(135,82,254,.1)' }}>
          <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#6f668f', textTransform: 'uppercase', letterSpacing: '.06em' }}>{t('confirmation.bookingRef')}</span>
        ...
        {confirmation?.customer?.email && (
          <p style={{ fontSize: '0.88rem', color: '#6f668f', margin: 0 }}>
            {t('confirmation.emailSentTo')} <strong style={{ color: '#1a1230' }}>{confirmation.customer.email}</strong>
          </p>
        )}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {total > 0 && (
            <div style={{ padding: '18px 20px', borderRadius: 18, background: 'rgba(255,255,255,.92)', border: '1px solid rgba(135,82,254,.12)', boxShadow: '0 4px 14px rgba(135,82,254,.06)' }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#6f668f', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>{t('confirmation.tripTotal')}</div>
          ...
          {dueNow >= 0 && (
            <div style={{ padding: '18px 20px', borderRadius: 18, background: dueNow > 0 ? 'rgba(110,73,255,.06)' : 'rgba(22,163,74,.06)', border: `1px solid ${dueNow > 0 ? 'rgba(110,73,255,.16)' : 'rgba(22,163,74,.18)'}`, boxShadow: '0 4px 14px rgba(135,82,254,.06)' }}>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#6f668f', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>{t('confirmation.dueNow')}</div>
              <div style={{ fontWeight: 900, fontSize: '1.4rem', color: dueNow > 0 ? '#4c1d95' : '#15803d' }}>{dueNow > 0 ? fmtMoney(dueNow) : t('confirmation.nothing')}</div>
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        <h2 style={{ fontSize: '1.05rem', fontWeight: 800, color: '#1a1230', margin: 0 }}>{t('confirmation.nextSteps')}</h2>
        {/* Primary action */}
        {primaryAction?.link && (
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', paddingTop: 8 }}>
        <button onClick={() => { if (typeof window !== 'undefined') window.print(); }} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 46, padding: '0 22px', borderRadius: 14, border: '1.5px solid rgba(110,73,255,.18)', background: 'rgba(255,255,255,.9)', color: '#4a38be', fontWeight: 800, fontSize: '0.93rem', cursor: 'pointer' }}>{t('confirmation.printReceipt')}</button>
        <Link href={withSiteBase(basePath, '/account')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 46, padding: '0 22px', borderRadius: 14, border: '1.5px solid rgba(110,73,255,.18)', background: 'rgba(255,255,255,.9)', color: '#4a38be', fontWeight: 800, textDecoration: 'none', fontSize: '0.93rem' }}>{t('confirmation.myTrips')}</Link>
        <Link href={withSiteBase(basePath, '/car-sharing')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 46, padding: '0 22px', borderRadius: 14, border: '1.5px solid rgba(110,73,255,.18)', background: 'rgba(255,255,255,.9)', color: '#4a38be', fontWeight: 800, textDecoration: 'none', fontSize: '0.93rem' }}>{t('confirmation.browseMore')}</Link>
      </div>
