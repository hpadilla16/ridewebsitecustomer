...
export default function HostReviewsPage() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <h1 style={{ margin: '4px 0 24px', fontSize: 'clamp(1.3rem, 3vw, 1.8rem)', fontWeight: 800, color: '#1e2847' }}>Guest Reviews</h1>

      {loading && <div className="surface-note" style={{ textAlign: 'center', color: '#6b7a9a' }}>Loading reviews...</div>}
      ...
      {!loading && profile && (
        <div className="glass card-lg" style={{ padding: '20px 22px', marginBottom: 24, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '0.84rem', color: '#6b7a9a', marginTop: 6 }}>{profile.reviewCount || 0} total reviews</div>
        </div>
      )}
        ...
        <section key={review.id} className="glass card" style={{ padding: '18px 20px', marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <div>
              ...
              <span style={{ marginLeft: 8, fontWeight: 700, color: '#1e2847', fontSize: '0.95rem' }}>{review.reviewerName || 'Guest'}</span>
            </div>
          ...
          {review.comments ? (
            ...
            <p style={{ margin: 0, color: '#9ca3af', fontSize: '0.88rem', fontStyle: 'italic' }}>No written review</p>
          )}
        </section>
      ))}

      {!loading && !reviews.length && !error && (
        <div className="surface-note" style={{ textAlign: 'center', color: '#6b7a9a' }}>No reviews yet. Reviews will appear here after guests rate your trips.</div>
      )}
    </div>
  );
}
